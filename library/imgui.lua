...
if not game:GetService("RunService"):IsStudio() and game:GetService("CoreGui"):FindFirstChild("imgui2") then
...
do -- Load items
	...
	imgui2.Name = "imgui2"
	imgui2.Parent = game:GetService("RunService"):IsStudio() and script.Parent.Parent or game:GetService("CoreGui")
	...
	SubHeading.ZIndex = 3
	SubHeading.Font = Enum.Font.Code
...
local CoreGui = game:GetService("RunService"):IsStudio() and script.Parent.Parent or game:GetService("CoreGui")
...
local library library = {
	new = function(options)
		...
		options = settings.new({
			...
			hidebutton = Enum.KeyCode.LeftControl,
		}).handle(options)
		...
		UserInputService.InputBegan:Connect(function(key)
			if key.KeyCode == options.hidebutton then
		...
		function self.new(tabOptions)
			...
			local types = { } do
				function types.label(labelOptions)
					...
					function self.setText(text, tween)
						label.Text = text
						label.Size = UDim2.new(0, label.TextBounds.X, 0, label.Size.Y.Offset)
					end
				...
				function types.switch(switchOptions)
					...
					text.TextColor3 = switchOptions.textcolor 
					...
					function self.set(boolean)
						...
						if not self.eventBlock then
							...
							switchOptions.callback(self.on)
						end
					end
				...
				function types.slider(sliderOptions)
					...
					sliderOptions = settings.new({
						...
						value = 0,
						float = 1,
						color = tint(options.color),
						barcolor = bleach(options.color),
						rounding = .1 or options.rounding,
						animation = options.animation,
						callback = function() end,
					...
					local function round2(num, bracket)
						...
						spawn(function()
							if inside and findBrowsingTopMost() == main then
								while mouse.held do
									...
									local v = round2(x / m, sliderOptions.float)
				...
				function types.dropdown(dropdownOptions)
					...
					dropdownOptions = settings.new({
						...
						color = tint(options.color),
						rounding = options.rounding,
						selectioncolor = tint(options.color),
						callback = function() end,
						list = {},
						value = "",
					...
					inner:FindFirstChild("Value").TextWrapped = true
					...
					function self.new(name)
						...
						if rawget(dropdownObjects, name) then return dropdownObjects[name] end
					...
					function self.search(key)
						for name, dropdownObject in next, dropdownObjects do
							...
							if dropdownObject.name:lower():match(key:lower()) then
					...
					do -- search bar
						local Inner = dropdownWindow:FindFirstChild("Content"):FindFirstChild("Search"):FindFirstChild("Outer"):FindFirstChild("Inner")
					...
					do -- load selected & list
						if dropdownOptions.value ~= "" then
							self.new(dropdownOptions.value):Select()
						end

						for a, b in dropdownOptions.list do
							self.new(b)
						end
					end

					self.self = dropdownButton
					self.list = dropdownObjects
					self.close()
					return self
				end
				
				function types.multdropdown(dropdownOptions)
					local self = { }
					self.isopen = true
					self.visible = false
					self.selected = nil
					...
					function self.new(name)
						...
						local multidropdownOption = new("DropdownOption")
						dropdownObject.object = multidropdownOption
						local content = dropdownWindow:FindFirstChild("Content")
						multidropdownOption.Parent = dropdownItems
						multidropdownOption.Text = "  " .. name
						multidropdownOption.TextColor3 = Color3.fromRGB(178, 178, 178)
						multidropdownOption.MouseButton1Click:Connect(function()
							if findBrowsingTopMost() == dropdownWindow then
								dropdownObject.Select()
							end
						end)
						...
						function dropdownObject.Select()
							...
							if self.enabled[name] then
								table.insert(self.multiselected, name)
								dropdownObjects[name].selected = true
								resize(multidropdownOption, { TextColor3 = Color3.new(1, 1, 1) }, 0.1)
								resize(multidropdownOption:GetChildren()[1], { ImageColor3 = dropdownOptions.selectioncolor }, 0.1)
							else
								local found = table.find(self.multiselected, name)
								if found then
									table.remove(self.multiselected, found)
								end
								resize(multidropdownOption, { TextColor3 = Color3.fromRGB(178, 178, 178) }, 0.1)
								resize(multidropdownOption:GetChildren()[1], { ImageColor3 = Color3.fromRGB(42, 44, 46) }, 0.1)
							end

							inner:FindFirstChild("Value").Text = string.format("[ %s ]", table.concat(self.multiselected, ", "))
							dropdownWindow:FindFirstChild("Content"):FindFirstChild("Selected").Text = string.format("[ %s ]", table.concat(self.multiselected, ", "))

							if not self.eventBlock then
								self.event:Fire(name)
								dropdownOptions.callback(self.multiselected)
								-- self.toggle(false)								
							end
						end
						...
						function dropdownObject.Clear()
							local is_value = table.find(self.multiselected, name)
							if is_value then
								inner:FindFirstChild("Value").Text = string.format("[ %s ]", table.concat(self.multiselected, ", "))
								dropdownWindow:FindFirstChild("Content"):FindFirstChild("Selected").Text = string.format("[ %s ]", table.concat(self.multiselected, ", "))
								table.remove(self.multiselected, is_value)
							end
							
							multidropdownOption:Destroy()
							rawset(dropdownObjects, name, nil)
						end
					...
					do -- load selected & list
						for a, b in dropdownOptions.list do
							if table.find(dropdownOptions.value, b) then
								self.new(b):Select()
								continue
							end
							self.new(b)
